...
public class LevelManager : MonoBehaviour
{
    ...
    public ItemBehavior[] items;   // on déclare un tableau d'objet qui prend moins de place que la liste et ne varie pas avec le temps dc moins utile de faire avec liste
    ...
    private void GiveMaterials()
    {
        ...
        for (int i = 0; i < row * column; i++)           // foreach non car prend bcp de place ds la mémoire surtt jeu video -> attention au FPS(frame par sec de l'application)
        {
            possibilities.Add(i);
        }
        ...
        {
            if (possibilities.Count < 2) break;   // sécurité si declarer plus de materiaux que d'objet
            ...
            itemMaterial.Add(id1, materials[i]);           // pour être sur que l'identifiant 1 et 2 ont le même material
